# Лабораторная работа №3 — ответы по пунктам 1–4 (+ повышенная сложность)

**Вариант использования (use case):** *«Построить прогноз временного ряда»*  
**Контекст системы:** десктопное Python‑приложение (PyQt6) + локальный backend (FastAPI) + локальная БД (PostgreSQL/SQLite) + файловая система (CSV/модели/отчёты).
...
## 2) Диаграмма последовательностей (use case)

![diagram-posled](diagram-posled.png)

## 3) Модель БД (UML диаграмма классов)
